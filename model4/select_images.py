...
for filename in os.listdir(image_dir):
    if filename.endswith(".jpg") or filename.endswith(".png"):
        ...
        # Load and prepare image for wildfire model
        ...
        wildfire_prediction = model.predict(img_array)[0][0]
        
        # Store results including wildfire probability
        results.append({
            'image_id': filename,
            'wildfire_probability': wildfire_prediction,
...
# Sort images by wildfire probability in descending order
sorted_df = df.sort_values(by='wildfire_probability', ascending=False)
...
def display_images(images_list, image_dir):
    ...
    for i, image_name in enumerate(images_list):
        ...
        plt.imshow(img)
        plt.title(f'Prob: {sorted_df.loc[sorted_df["image_id"] == image_name, "wildfire_probability"].iloc[0]:.2f}')
        plt.axis('off')
    plt.tight_layout()
    plt.show()
